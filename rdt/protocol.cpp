...
packet frame_to_packet(frame *frm)
{
    ...
    memcpy(p.data + HEADERSIZE, frm->info, frm->size);
    unsigned int checksum = crc32(p.data, RDT_PKTSIZE - RDT_CHECKSUM_SIZE);
    memcpy(p.data + HEADERSIZE + PAYLOADSIZE, (char *)&checksum, RDT_CHECKSUM_SIZE);
...
{
    unsigned int checksum = crc32(pkt->data, HEADERSIZE + PAYLOADSIZE);
    unsigned int footer_checksum = *(unsigned int *)(pkt->data + HEADERSIZE + PAYLOADSIZE);

    return checksum == footer_checksum;
}
